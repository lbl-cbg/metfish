...
CKPT_PATH="/pscratch/sd/s/smprince/projects/metfish/model_outputs/checkpoints/afsaxs_nma/epoch=7-step=8280-v1.ckpt"
CKPT_ARGS=""
if [[ -n "$CKPT_PATH" ]]; then
...
# srun -n $TOTAL_TASKS python $TRAIN_SCRIPT $DATA_DIR $OUTPUT_DIR --gpus_per_node $GPUS_PER_NODE --num_nodes $NUM_NODES --job_name $SLURM_JOB_NAME $CKPT_ARGS --use_l1_loss --unfreeze_af_weights --jax_param_path ""